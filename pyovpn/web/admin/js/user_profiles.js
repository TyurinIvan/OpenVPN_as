...
async function copyTokenUrlToClipboard() {
  /* Get the text field */
  var copyText = document.getElementById("profileTokenUrl");
...
function createNewTokenURL() {
    $('#createToken').prop('disabled', true);
    user = $('#tokenurl_username').val();
    type = $('input[name=tokenurl-type]:checked').val();
    expiry = $('#profile-expiry').val();

    $.post(rootURL + "/create_new_token_url", {
        'user': user,
        'type': type,
        'expiry': expiry,
    },
        function(tokenurl, status, xhr) {
            authenticated = xhr.getResponseHeader('Is-Authenticated')
            if (authenticated == 'false') {
                window.location.reload();
            }
            else {
                $("#tokenUrlResult").show();
                tokenField = document.getElementById('profileTokenUrl');
                tokenField.value = tokenurl;
            }
        });
}
...
function createTokenUrlModal(user) {
    // fetch autologin permissions for this user
    $.post(rootURL + "/get_autologin", { 'user': user },
        function(result) {
            // show modal with autologin permissions
            $('#tokenurlModalLabel').text('Create Token URL for ' + user);
            document.getElementById('tokenUrlResult').style.display = "none";
            $('#tokenurl_username').val(user);

            if (result == 'false') {
                $('#tokenurl-type-autologin').attr('disabled', 'true');
                $('#tokenurl-type-autologin').attr('checked', 'false');
                $('#tokenurl-type-userlocked').prop('checked', 'true');
            } else {
                $('#tokenurl-type-autologin').removeAttr('disabled');
                $('#tokenurl-type-autologin').attr('checked', 'true');
                $('#tokenurl-type-userlocked').prop('checked', 'false');
            }
            $('#createToken').prop('disabled', false);
            $('#tokenurlModal').modal('show');
        }
    );
}
...
function render_new_tokenurl(data, type, row) {
    return '<button onclick="createTokenUrlModal(\'' + data + '\');" type="button" class="btn btn-outline-secondary"><i class="fal fa-plus-circle"></i> New Token Url</button>';
}
